...
static void blec_background_callback()
{
    singleten->background_callback();
}

bool tble::tdisable_reconnect_lock::require_reconnect = false;
	...
	, persist_xmit_audio_lock_(NULL)
{
	...
	callbacks_.background_callback = blec_background_callback;
...
void tble::background_reconnect_detect(bool start)
{
	if (start) {
		...
		background_scan_ticks_ = SDL_GetTicks() + threshold;
		SDL_XmitAudio(SDL_TRUE);

		VALIDATE(!persist_xmit_audio_lock_, null_str);
		persist_xmit_audio_lock_ = new sound::tpersist_xmit_audio_lock;
	} else {
...
void tble::background_callback()
{
	if (background_scan_ticks_ && SDL_GetTicks() >= background_scan_ticks_) {
		if (!Mix_PlayingMusic()) {
