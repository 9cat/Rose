...
*/
    ...
    public static int audioInit(int sampleRate, boolean is16Bit, boolean isStereo, int desiredFrames) {
        if (mPlaybackAudioService != null && mPlaybackAudioService.mExitThread) {
            Log.i(TAG, "audioInit, status error, mPlaybackAudioService must be null when audioInit.");
            return -1;
        }
        if (sampleRate < 8000 || sampleRate > 48000 || desiredFrames == 0 || ((desiredFrames & 0x3ff) != 0)) {
            Log.i(TAG, "audioInit, invalid parameter.");
            return -1;
        }

        Log.i(TAG, "audioInit, begin to create PlaybackAudio Service.");
...
*/
            mPlaybackAudioService = null;
            Log.i(TAG, "audioQuit, set mPlaybackAudioService to null.");
        } else {
            Log.i(TAG, "audioQuit, mPlaybackAudioService is null, do nothing.");
        }
    }
