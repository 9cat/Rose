...
void treport::tabbar_init(bool toggle, const std::string& definition, bool always, int width, int height, int gap)
{
	...
	previous_->set_surface(image::get_image("misc/arrow-left.png"), unit_size_.x, unit_size_.y);
	...
	next_->set_surface(image::get_image("misc/arrow-right.png"), unit_size_.x, unit_size_.y);
	connect_signal_mouse_left_click(
		*next_
		, boost::bind(
			&treport::click
...
{
	VALIDATE(!content_grid_->children_vsize(), "Duplicate call!");
	VALIDATE(multi_line_, "It is tabbar report, cannot call multiline_init!");
